...
{
	float32_t RadiusMin;			// [m]
	float32_t RadiusMax;			// [m]
	float32_t TargetVelocityMin;	// [m/s]
...
static StrCourceLog st_DrivePlan;
...
static LOG_StrCourceLogArray st_PlanArray[PLAN_MAX];

static void st_SetAnalysisParam(StrCourceAnalysisParam param);
static StrCourceAnalysisParam st_GetAnalysisParam(void);
...
static void st_AnalyzeCourceByMarker(void);
...
void TRC_Init(void)
{
	...
	analysisParam.RadiusMin = 0.1;
	analysisParam.RadiusMax = 100.0;
	analysisParam.TargetVelocityMin = 1.4;
	analysisParam.TargetVelocityMax = 2.0;
	//analysisParam.CentrifugalMax = 20.0;
...
//	st_CourceAnalysisParam.CentrifugalMax = 40.0;
	...
	for(uint32_t i = 0; i < PLAN_MAX; ++i)
	{
		st_PlanArray[i].AngularVelocity = 0.0;
		st_PlanArray[i].Distance = 0.0;
		st_PlanArray[i].MarkerKind = SSR_COURCE_MARKER_NON;
		st_PlanArray[i].Radius = 0.0;
		st_PlanArray[i].SensorAngle = 0.0;
		st_PlanArray[i].Velocity = 0.0;
		st_PlanArray[i].a_n = 0.0;
		st_PlanArray[i].s_n = 0.0;
		st_PlanArray[i].v_n = 0.0;
	}
}
...
//		SCF_WriteData(st_SendBuf, st_BufSize);
		...
		{
			++vecCount;
			// 2019/09/08 東日本大会では動作してなかった
			//if(vecCount >= 5){
				CVL_SetTarget(rate * vecTargetTemp);
				vecFlag = 1;
			//}
		}
...
//		SCF_WriteData(st_SendBuf, st_BufSize);
		...
		*/
		...
		{
			StrCourceAnalysisParam analysisParam = st_GetAnalysisParam();
			...
			// コース解析
			st_PrepareCource();
			analysisParam.TargetVelocityMin = 1.0;
			analysisParam.TargetVelocityMax = 2.0;
			CVL_SetTargetUpAccel(3.0);
			CVL_SetTargetDownAccel(3.0);
			st_SetAnalysisParam(analysisParam);
			st_AnalyzeCourceByMarker();
...
void TRC_RecordCourceTask(void)
{
	...
	{
		if(st_CourceLogStoreParam.MarkerData.MarkerKind == SSR_COURCE_MARKER_RIGHT)
		{
...
void TRC_PlayCourceTask(void)
{
	...
	float32_t distance;
	int32_t indexDif;
	float32_t downAccelBuff = -CVL_GetDownAccel();
	float32_t sensorAngle;
	...
	if(st_CourceLog.Pt_Array[st_CourceLog.Index].BrakeDistance > 0.0)
	{
		velocity = CVL_GetVelocity();
		float32_t brakeDistance = (powf(st_CourceLog.Pt_Array[st_CourceLog.Index+1].TargetVelocity, 2) - powf(velocity, 2)) / (2.0 * downAccelBuff);
		if(brakeDistance > st_CourceLog.Pt_Array[st_CourceLog.Index].BrakeDistance)
		{
			distance = st_CourceLog.Pt_Array[st_CourceLog.Index].Distance - 1.2 * brakeDistance;
		}
		else
		{
			distance = st_CourceLog.Pt_Array[st_CourceLog.Index].Distance - 1.2 * st_CourceLog.Pt_Array[st_CourceLog.Index].BrakeDistance;
		}

		if(st_CourceLogStoreParam.DiffDistance > distance)
		{
			velocity = st_CourceLog.Pt_Array[st_CourceLog.Index + 1].TargetVelocity;
			CVL_SetTarget(velocity);
		}
	}
//	else
//	{
//		distance = st_CourceLog.Pt_Array[st_CourceLog.Index].Distance;
//	}

	if(st_CourceLogStoreParam.DiffDistance > 1.5 * st_CourceLog.Pt_Array[st_CourceLog.Index].Distance + 0.05)
	{
//		++st_CourceLog.Index;
...
//
//		st_CourceLogStoreParam.StartDistance = st_CourceLogStoreParam.TempDistance;
//		velocity = st_CourceLog.Pt_Array[st_CourceLog.Index].TargetVelocity;
////		if(velocity > st_CourceLog.Pt_Array[st_CourceLog.Index - 1].TargetVelocity)
////		{
////			if(st_CourceLog.Pt_Array[st_CourceLog.Index].Distance < 0.15)
////			{
////				CVL_SetTargetUpAccel(3.0);
////			}
////			else
////			{
////				CVL_SetTargetUpAccel(6.0);
////			}
////		}
//		CVL_SetTarget(velocity);
//
...
static void st_SetAnalysisParam(StrCourceAnalysisParam param)
{
	param.CentrifugalMax = powf(param.TargetVelocityMin, 2) / param.RadiusMin;
...
static void st_PrepareCource(void)
{
	...
	st_DrivePlan.Pt_Array = st_PlanArray;
...
static uint16_t markerIndexList[1000] = {0};
static void st_AnalyzeCourceByMarker(void)
{
	SSR_EnmCourceMarkerKind markerKind;
	int32_t averageNum = 10;
	int32_t averageOffset = 5;
	float32_t velocityBuff;
	float32_t angularBuff;
	float32_t radiusBuff;
	float32_t upAccelBuff;
	...
	for(i = 0; i < st_CourceLog.Index; ++i)
	{
		if(st_CourceLog.Pt_Array[i].MarkerKind != 0)
		{
//			st_BufSize = sprintf(st_SendBuf, "[%d]: %d \r\n", j, i);
//			SCF_WriteData(st_SendBuf, st_BufSize);
			markerIndexList[j++] = i;
		}
	}
	markerIndexList[j++] = st_CourceLog.Index - 1;

	st_CourceLog.IndexMax = j;

	for(i = 0; i < st_CourceLog.IndexMax; ++i)
	{
		st_BufSize = sprintf(st_SendBuf, "[%d]: %d \r\n", i, markerIndexList[i]);
		SCF_WriteData(st_SendBuf, st_BufSize);
	}

	for(i = 1; i < st_CourceLog.IndexMax; ++i)
	{
		float32_t distance = st_CourceLog.Pt_Array[markerIndexList[i]].Distance - st_CourceLog.Pt_Array[markerIndexList[i-1]].Distance;
		uint16_t srcCount = markerIndexList[i-1];
		uint16_t dstCount = markerIndexList[i];
		uint16_t avgCount = dstCount - srcCount;

		float32_t radAvg = 0.0;
		for(k = 0; k < avgCount; ++k)
		{
			radAvg += st_CourceLog.Pt_Array[srcCount + k].Radius;
		}
		...
		st_CourceLog.Pt_Array[i-1].Distance = distance;
		st_CourceLog.Pt_Array[i-1].Radius = radAvg;

		st_BufSize = sprintf(st_SendBuf, "[%d]: %.2f %.2f \r\n", i-1, distance, radAvg);
		SCF_WriteData(st_SendBuf, st_BufSize);
	}
	...
	st_CourceLog.Pt_Array[st_CourceLog.Index - 1].TargetVelocity = 0.0;
	...
	float32_t distance = 0.5;
	float32_t velocity = sqrtf(2.0 * fabs(downAccelBuff) * distance);
	float32_t radAvg = 100.0 * powf(velocity, 2) / st_CourceAnalysisParam.CentrifugalMax;
	float32_t radInvAvg = 100.0 / radAvg;

	//st_CourceLog.Pt_Array[st_CourceLog.Index].Distance = distance;
	//st_CourceLog.Pt_Array[st_CourceLog.Index].Radius = radAvg;
	//st_CourceLog.Pt_Array[st_CourceLog.Index].TargetVelocity = velocity;

	for(i = 0; i < st_CourceLog.Index; ++i)
	{
		float32_t brake_dif = 0.0;
		j = i;
		while(brake_dif <= 0.0)
		{
			float32_t brakeDistance = (powf(st_CourceLog.Pt_Array[j+1].TargetVelocity, 2) - powf(st_CourceLog.Pt_Array[j].TargetVelocity, 2)) / (2.0 * downAccelBuff);
			brake_dif = brakeDistance - st_CourceLog.Pt_Array[j].Distance;
			if(brake_dif > 0.0)
			{
				st_CourceLog.Pt_Array[j].TargetVelocity = sqrtf(powf(st_CourceLog.Pt_Array[j+1].TargetVelocity, 2) - 2.0 * downAccelBuff * st_CourceLog.Pt_Array[j].Distance);
				st_CourceLog.Pt_Array[j].BrakeDistance = st_CourceLog.Pt_Array[j].Distance;
			}
			else
			{
				st_CourceLog.Pt_Array[j].BrakeDistance = brakeDistance;
			}
			--j;
			if(j <= 0)
			{
	...
	for(i = 0; i < st_CourceLog.Index; ++i)
	{
		if(st_CourceLog.Pt_Array[i].BrakeDistance > 0.0)
		{
			float32_t targetVelocityTemp = sqrtf(powf(st_CourceLog.Pt_Array[i+1].TargetVelocity, 2) + 2.0 * fabs(downAccelBuff) * st_CourceLog.Pt_Array[i].Distance);
			if(st_CourceLog.Pt_Array[i].TargetVelocity > targetVelocityTemp)
			{
				st_CourceLog.Pt_Array[i].TargetVelocity = targetVelocityTemp;
			}
		}
	}
	...
	for(i = 0; i < st_CourceLog.Index; ++i)
	{
		float32_t brakeDistance = (powf(st_CourceLog.Pt_Array[i+1].TargetVelocity, 2) - powf(st_CourceLog.Pt_Array[i].TargetVelocity, 2)) / (2.0 * downAccelBuff);
		st_CourceLog.Pt_Array[i].BrakeDistance = brakeDistance;
	}

	st_CourceLog.Pt_Array[st_CourceLog.Index + 1].Velocity = 0.0;
